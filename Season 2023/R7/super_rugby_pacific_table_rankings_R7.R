...
super_rugby_rankings_bump_chart <- super_rugby_2023_data |>
  ...
  geom_from_path(data = super_rugby_2023_data |>
                   filter(Round == 7),
                 aes(x = 7.5, y=Ranking, path = logo), width = 0.05) +
  annotate(
    "text",
    x = c(1, 7),
    y = c(0.25, 0.25),
    label = c("Round 1", "Round 7"),
    hjust = c(0, 1), vjust = 1, size = 4, fontface = "bold", colour = "#0e0e0b", 
    family = "Roboto Condensed"
  ) +
  scale_y_reverse(position = "right", breaks = seq(16, 2, -2)) +
  scale_color_manual(values = team_colors) +
  coord_cartesian(xlim = c(0.05, 8), ylim = c(12.5, 0.05), expand = F) +
  theme_minimal() +
  theme(
    ...
    plot.title = element_text(
      face = "bold", colour = "#0e0e0b", family = "Roboto Condensed", size = 18, hjust = 0.5
    ),
    plot.caption = element_textbox_simple(
      lineheight = 0.4, colour = "#0e0e0b", linewidth = 9.5, 
  ...
  labs(
    x = "",
    y = "",
    title = "\n SUPER RUGBY PACIFIC 2023: Weekly Evolution of the Leaderboard",
    subtitle = "No. of Rounds Complete: 7",
    caption = cap
  )

# Save plot
ggsave("super_rugby_rankings_2023_R7.png", width = 8.5, height = 7, bg = "white")
