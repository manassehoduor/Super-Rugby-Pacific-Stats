...
Tries_Players <- import("super_rugby_2023_tries_R7.xlsx", which="Tries_Players")
Tries_Team <- import("super_rugby_2023_tries_R7.xlsx", which="Tries_Team")
Pen_Try <- import("super_rugby_2023_tries_R7.xlsx", which="Pen_Try")
...
subtitle = "<span style='font-size:13pt;'> At the conclusion of the 7th round, Shaun Stevenson, Leicester Fainga'anuku, and Jordan Petaia are <br> currently tied as the top try scorers, with each having scored 7 tries </span>"
...
cap <-  paste0("<span style='font-family:fb;'>&#xf09b;</span>",
               ...
               "<span style='font-family:fb;'>&#xf099; </span>  Manasseh_6 | #SuperRugbyPacific @R7")
# Labels
...
ggplot(Top_Tries_Scorers) +
  ...
  annotate(geom = "text", x = 6.1, y = 4, label = "Tevita Ikanivere is the only Forward player\n in the top try scorers list  with 5 tries", 
           hjust = "left", color = "#fafaf9", size = 4, family = "Rosario")

# Save plot
ggsave("super_rugby_2023_top_tries_scorers_R7.png", width = 10, height = 7.5)
...
Back_Forward_Tries_tbl = Back_Forward_Tries_P_List_data %>%
  ...
  cols_width(
    ...
    Tries ~ px(40),
    Logo ~ px(50)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("*2023 Season:* Aggregate tries scored by the team, with a breakdown of back and forward tries, after *Round 7*")) %>%
  ...
  cols_align(
    ...
    columns = Tries) %>% 
  tab_source_note(
    source_note = md("**Table:** Manasseh Oduor | **Source:** super.rugby | **#SuperRugbyPacific:** @R7"))
...
# Save table
new_gt_save(Back_Forward_Tries_tbl, "super_rugby_2023_B_F_tries_R7.png")
...
First_Second_Half_Tries_tbl = First_Second_Half_Tries_List_data %>% 
  ...
  cols_width(
    ...
    Tries ~ px(40),
    Logo ~ px(50)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("*2023 Season:* Aggregate tries scored by the team, with a breakdown of 1st Half and 2nd Half tries, after *Round 7*")) %>%
...
# Save table
new_gt_save(First_Second_Half_Tries_tbl, "super_rugby_2023_F_S_tries_R7.png")
