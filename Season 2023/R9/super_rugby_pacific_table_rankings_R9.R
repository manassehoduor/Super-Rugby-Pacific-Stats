...
super_rugby_rankings_bump_chart <- super_rugby_2023_data |>
  ...
  geom_from_path(data = super_rugby_2023_data |>
                   filter(Round == 9),
                 aes(x = 9.5, y=Ranking, path = logo), width = 0.05) +
  annotate(
    "text",
    x = c(1, 9),
    y = c(0.25, 0.25),
    label = c("Round 1", "Round 9"),
    hjust = c(0, 1), vjust = 1, size = 4, fontface = "bold", colour = "#313131", 
    family = "Roboto Condensed"
  ) +
  scale_y_reverse(position = "right", breaks = seq(16, 2, -2)) +
  scale_color_manual(values = team_colors) +
  coord_cartesian(xlim = c(-0.35, 10), ylim = c(12.5, 0.05), expand = F) +
  theme_minimal() +
  theme(
  ...
  labs(
    ...
    subtitle = "Weekly Evolution of the Standings following Round 9",
    caption = cap
  )

# Save plot
ggsave("super_rugby_rankings_2023_R9.png", width = 8.5, height = 7.5, bg = "#fbf5c6")
