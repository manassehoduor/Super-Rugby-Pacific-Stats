...
Tries_Players <- import("super_rugby_2023_tries_R9.xlsx", which="Tries_Players")
Tries_Team <- import("super_rugby_2023_tries_R9.xlsx", which="Tries_Team")
Pen_Try <- import("super_rugby_2023_tries_R9.xlsx", which="Pen_Try")
...
subtitle = "<span style='font-size:15pt;'> At the conclusion of the 9th round, Shaun Stevenson is currently leading the try scorer chart having scored 8 tries<br></span>"
...
cap <-  paste0("<span style='font-family:fb;'>&#xf09b;</span>",
               ...
               "<span style='font-family:fb;'>&#xf099; </span>  Manasseh_6 | #SuperRugbyPacific @R9")
# Labels
...
ggplot(Top_Tries_Scorers) +
  ...
  annotate(geom = "text", x = 6.1, y = 5, label = "Ricky Riccitelli of Blues is the only forward in the top try scorers \n list with 6 tries.", 
           hjust = "left", color = "#fafaf9", size = 4.5, family = "Rosario")

# Save plot
ggsave("super_rugby_2023_top_tries_scorers_R9.png", width = 10, height = 8)
...
Back_Forward_Tries_tbl = Back_Forward_Tries_P_List_data %>%
  ...
  cols_width(
    ...
    Tries ~ px(40),
    Logo ~ px(50)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("*2023 Season:* Aggregate tries scored by the team, with a breakdown of backs and forwards tries, after *Round 9*")) %>%
  ...
  tab_source_note(
    source_note = md("**Table:** Manasseh Oduor | **Source:** super.rugby | **#SuperRugbyPacific:** @R9"))
...
# Save table
new_gt_save(Back_Forward_Tries_tbl, "super_rugby_2023_B_F_tries_R9.png")
...
First_Second_Half_Tries_tbl = First_Second_Half_Tries_List_data %>% 
  ...
  cols_width(
    ...
    Tries ~ px(40),
    Logo ~ px(50)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("*2023 Season:* Aggregate tries scored by the team, with a breakdown of 1st-Half and 2nd-Half tries, after *Round 9*")) %>%
...
# Save table
new_gt_save(First_Second_Half_Tries_tbl, "super_rugby_2023_F_S_tries_R9.png")
