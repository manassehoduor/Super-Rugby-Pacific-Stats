...
# load data
scrums_lineouts <- import("scrums_lineouts.xlsx", which="R10")
...
# plot
  scrums_lineouts |>
    ...
    geom_text(aes(y=-Scrums_Won, label = glue("{Scrums_Won}%")), vjust = 0.5, hjust = -2, 
              color = "white", size = 3, nudge_x = -0.15) +
    geom_text(aes(y = -110, label = glue("{round(avg_scrum_lineouts, 1)}%")), fontface = "bold",
              vjust = 0.5, color = "#6f5d5f", size = 3.5) +
    coord_flip() +
    theme_minimal() +
    labs(
      title = 'SUPER RUGBY PACIFIC\nCUMULATIVE SCRUMS & LINE-OUTS SUCCESS RATE',
      subtitle = "<span style='color:#3d8d2c;'>**Lineout**</span> Brilliance: At Round 10, Queensland Reds triumph with 90.7% success rate.<br> Chiefs <span style='color:#12369d;'>**Scrum**</span> Prowess with 90% success rate.<br>Hurricanes tops in the cumulative success rate (87.8%) of lineouts and scrums combined averages<br>",
      caption = cap
    ) +
    theme_void() +
    theme(
      ...
      plot.subtitle = element_markdown(hjust = 0, size = 13, color = "black"),
      plot.background = element_rect(fill = "white", colour = "white"),
      plot.caption = element_markdown(colour = 'black', hjust = 0.5, size = 10,
                                      family = 'Rosario', margin = margin(t = 5)),
      plot.margin = margin(b = 10, t = 20, r = 20, l = 10)
    )
  
# Save plot
ggsave("super_rugby_2023_lineout_scrums_R10.png", width = 9, height = 8, bg = "#f7f6f6")
