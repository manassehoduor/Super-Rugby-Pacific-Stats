...
# Round 12
table_data <- update_table("Chiefs", "Reds", 22, 25, 3, 4, table_data)
table_data <- update_table("Force", "Fijian Drua", 34, 14, 5, 2, table_data)
table_data <- update_table("Hurricanes", "Moana Pasifika", 71, 22, 11, 3, table_data)
table_data <- update_table("Crusaders", "Blues", 15, 3, 2, 0, table_data)
table_data <- update_table("Waratahs", "Rebels", 38, 20, 5, 2, table_data)
table_data <- update_table("Brumbies", "Highlanders", 48, 32, 7, 4, table_data)

# Round 13
table_data <- update_table("Moana Pasifika", "Crusaders", 7, 41, 1, 6, table_data)
table_data <- update_table("Reds", "Blues", 26, 45, 4, 6, table_data)
table_data <- update_table("Highlanders", "Rebels", 20, 17, 2, 2, table_data)
table_data <- update_table("Chiefs", "Hurricanes", 23, 12, 3, 2, table_data)
table_data <- update_table("Waratahs", "Fijian Drua", 32, 18, 5, 2, table_data)
table_data <- update_table("Force", "Brumbies", 34, 19, 4, 3, table_data)
...
super_rugby_2023 = table_data %>%
  ...
  cols_width(
    ...
    Logo ~ px(45),
    Pos ~ px(35)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("2023 Season Standings after Round 13")) %>%
  ...
  tab_source_note(
    source_note = md("**Table:** Manasseh Oduor | **Source:** super.rugby | **#SuperRugbyPacific:** @R13"))
...
# Save table
new_gt_save(super_rugby_2023,"super_rugby_standings_2023_R13.png")
