...
cap <-  paste0("<span style='font-family:fb;'>&#xf09b;</span>",
               ...
               "<span style='font-family:fb;'>&#xf099; </span> Manasseh_6 | Source: super.rugby | #SuperRugbyPacific @R13")
...
super_rugby_rankings_bump_chart <- super_rugby_2023_data |>
  ...
  geom_from_path(data = super_rugby_2023_data |>
                   filter(Round == 13),
                 aes(x = 13.7, y=Ranking, path = logo), width = 0.05) +
  annotate(
    "text",
    x = c(1, 13),
    y = c(0.25, 0.25),
    label = c("Round 1", "Round 13"),
    hjust = c(0, 1), vjust = 1, size = 4, fontface = "bold", colour = "#313131", 
    family = "Roboto Condensed"
  ) +
  scale_y_reverse(position = "right", breaks = seq(16, 2, -2)) +
  scale_color_manual(values = team_colors) +
  coord_cartesian(xlim = c(-0.58, 14.5), ylim = c(12.5, 0.05), expand = F) +
  theme_minimal() +
  theme(
    ...
    plot.subtitle = element_text(family = "Barriecito", colour = "#0e0e0b", size = 15,
                                 margin = margin(t = 3, b = 2, unit = "mm"), hjust = 0.5
    ),
    plot.title = element_text(
      face = "bold", colour = "#0e0e0b", family = "Rubik Beastly", size = 25, hjust = 0.5
    ),
  ...
  labs(
    ...
    subtitle = "Weekly Evolution of the Standings following Round 13",
    caption = cap
  )

# Save plot
ggsave("super_rugby_rankings_2023_R13.png", width = 10.5, height = 8.5, bg = "white")
