...
cap <-  paste0("<span style='font-family:fb;'>&#xf09b;</span>",
               ...
               "<span style='font-family:fb;'>&#xf099; </span> Manasseh_6 | Source: super.rugby | #SuperRugbyPacific @R12")
...
super_rugby_rankings_bump_chart <- super_rugby_2023_data |>
  ...
  geom_from_path(data = super_rugby_2023_data |>
                   filter(Round == 12),
                 aes(x = 12.7, y=Ranking, path = logo), width = 0.05) +
  annotate(
    "text",
    x = c(1, 12),
    y = c(0.25, 0.25),
    label = c("Round 1", "Round 12"),
    hjust = c(0, 1), vjust = 1, size = 4, fontface = "bold", colour = "#313131", 
    family = "Roboto Condensed"
  ) +
  scale_y_reverse(position = "right", breaks = seq(16, 2, -2)) +
  scale_color_manual(values = team_colors) +
  coord_cartesian(xlim = c(-0.55, 13), ylim = c(12.5, 0.05), expand = F) +
  theme_minimal() +
  theme(
  ...
  labs(
    ...
    subtitle = "Weekly Evolution of the Standings following Round 12",
    caption = cap
  )

# Save plot
ggsave("super_rugby_rankings_2023_R12.png", width = 10, height = 8.5, bg = "white")
