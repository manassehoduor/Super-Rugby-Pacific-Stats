...
# load libraries
pacman::p_load(knitr, tidyverse, rio, gt, gtExtras, extrafont, ggtext, 
               showtext, htmltools, patchwork, ggpath, webshot2)
...
Tries_Players <- import("super_rugby_2023_tries_R8.xlsx", which="Tries_Players")
Tries_Team <- import("super_rugby_2023_tries_R8.xlsx", which="Tries_Team")
Pen_Try <- import("super_rugby_2023_tries_R8.xlsx", which="Pen_Try")
...
Top_Tries_Scorers = Tries_Players_1 |>
  mutate(color = case_when(Position == "Forward"~'#65e8e8', TRUE~'#1e40af'),
         Team = toupper(Team)) |>
  head(14)


title = "<span style='font-size:26pt;'> SUPER RUGBY PACIFIC TOP TRY SCORERS</span>"
subtitle = "<span style='font-size:16pt;'> At the conclusion of the 8th round, Shaun Stevenson, Leicester Fainga'anuku, and Jordan Petaia are <br> currently tied as the top try scorers, with each having scored 7 tries </span>"
...
# Labels
labels = paste0("<span style='font-size:13pt;color:#09090b;'><b>", Top_Tries_Scorers$Player, "</b></span><br>",
                "<span style='font-size:12pt;color:#09090b;'><b>", Top_Tries_Scorers$Team, "</b><br></span>",
                "<span style='font-size:11pt;color:#09090b;'><b>", Top_Tries_Scorers$No_of_Tries, "</span>")
...
ggplot(Top_Tries_Scorers) +
  ...
  geom_richtext(aes(y=reorder(Player, No_of_Tries), x=No_of_Tries, label=labels, color=color),
                hjust=1, fill=NA, label.colour=NA, lineheight=0.7, family = "Roboto Condensed") +
  geom_segment(aes(x=-0.2, y=reorder(Player, No_of_Tries), xend=12, 
                   yend=reorder(Player, No_of_Tries)), color="NA") +
  scale_fill_manual(values=c('#f3f4f6', '#bfdbfe')) +
  ...
  theme(
        ...
        plot.subtitle = element_markdown(hjust=.1, family = "Rosario", color = "white"),
        plot.caption = element_markdown(size=11, hjust=.97, family = "Rosario", color = "white"),
        panel.grid = element_blank(),
        panel.background = element_rect(fill = "#991b1b", color = NA),
        plot.background = element_rect(fill = "#450a0a", color = NA),
        legend.position = 'none',
        axis.title = element_blank(),
        axis.text = element_blank())+
  annotate(
    geom = "curve", x = 5, y = 4, xend = 6.0, yend = 4.1, 
    curvature = -.3, arrow = arrow(length = unit(2, "mm")), color = 'white'
  ) +
  annotate(
    geom = "curve", x = 5, y = 5, xend = 6.0, yend = 4.3, 
    curvature = -.3, arrow = arrow(length = unit(2, "mm")), color = 'white'
  ) +
  annotate(geom = "text", x = 6.1, y = 4, label = "Tevita Ikanivere of Fijian Drua and Lachlan Lonergan of Brumbies are \n the only Forward players in the top try scorers list  with 5 tries each.", 
           hjust = "left", color = "#fafaf9", size = 4.5, family = "Rosario")

# Save plot
ggsave("super_rugby_2023_top_tries_scorers_R8.png", width = 11, height = 10)
...
Back_Forward_Tries_tbl = Back_Forward_Tries_P_List_data %>%
  ...
  cols_width(
    ...
    Tries ~ px(40),
    Logo ~ px(50)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("*2023 Season:* Aggregate tries scored by the team, with a breakdown of backs and forwards tries, after *Round 8*")) %>%
  ...
  cols_align(
    align = "center",
    columns = c(Team, Logo, Tries)) %>% 
  tab_source_note(
    source_note = md("**Table:** Manasseh Oduor | **Source:** super.rugby | **#SuperRugbyPacific:** @R8"))
...
# Save table
new_gt_save(Back_Forward_Tries_tbl, "super_rugby_2023_B_F_tries_R8.png")
...
First_Second_Half_Tries_tbl = First_Second_Half_Tries_List_data %>% 
  ...
  cols_width(
    ...
    Tries ~ px(40),
    Logo ~ px(50)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("*2023 Season:* Aggregate tries scored by the team, with a breakdown of 1st-Half and 2nd-Half tries, after *Round 8*")) %>%
...
# Save table
new_gt_save(First_Second_Half_Tries_tbl, "super_rugby_2023_F_S_tries_R8.png")
