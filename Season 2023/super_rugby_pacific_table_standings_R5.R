...
super_rugby_2023 = table_data %>%
  ...
  gt_theme_538() %>% 
  gt::data_color(
    columns = Pts, colors = c("white", "orange")) %>% 
  cols_width(
    Team ~ px(120),
    PD ~ px(50),
    Pts ~ px(40),
    Logo ~ px(50),
    Pos ~ px(35)) %>% 
  tab_header(title = md("**SUPER RUGBY PACIFIC**"), subtitle = md("*2023 Season:* Standings after *Round 5*")) %>%
  cols_label(Pos = "Pos", Team = "Team", P = "P", W = "W", D = "D", L = "L",
             PD = "PD", Pts = "Pts") %>%
  tab_options(
  ...
  cols_align(
    ...
    columns = Pos) %>% 
  tab_source_note(
    source_note = md("**Table:** _@ _Manasseh__  | **#SuperRugbyPacific:** @Round5"))
...
# Save table
new_gt_save(super_rugby_2023,"super_rugby_2023.png")
